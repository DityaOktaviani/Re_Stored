...
$(document).ready(function (){
  ...
  $('.cari').keyup(function () {
   ...
   //fungsi acc
   $(document).on('click', '.status_buku', function(){
    var id = $(this).closest('tr').find('.id_data').val();
    //console.log(id);
    Swal.fire({
      title: 'Apakah buku ini akan di ACC?',
      showDenyButton: true,
      ...
      denyButtonText: `Tidak`,
        ...
        if (result.isConfirmed) {
          ...
          Swal.fire('Buku batal di ACC', '', 'error')
        }
      })
        ...
        success:function (response){
            ...
            $.each(response.buku, function(key,value){
              ...
              if(value['dikonfirmasi'] == 1){
                status = '<a class="badge bg-danger status_buku"><i class="fas fa-book"></i> Draft</a>';
              }else{
